...
string Paivays::merkkijonoksi(string formaatti) const {
    map<string, string> muunnokset {
        ...
        // Nämä muokattu alkuperäisestä, jotta ohjelma tulostaa päivämäärät muodossa pp.kk.vvvv
        if ( muunnokset.at("%{PAIVA}").length() < 2){
            muunnokset.at("%{PAIVA}").insert(0, "0");
        }

        if ( muunnokset.at("%{KUU}").length() < 2){
            muunnokset.at("%{KUU}").insert(0, "0");
        }
